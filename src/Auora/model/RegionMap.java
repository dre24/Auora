...
public class RegionMap {
    ...
    public void setMask(int plane, int x, int y, int mask) {
        if (x >= 64 || y >= 64 || x < 0 || y < 0) {
            RSTile tile = RSTile.createRSTile(regionX + x, regionY + y, plane);
            int regionId = tile.getRegionId();
            int newRegionX = (regionId >> 8) * 64;
            int newRegionY = (regionId & 0xff) * 64;
            if (clipedOnly)
                RSObjectsRegion.getRegion((short) tile.getRegionId(), false).forceGetRegionMapClipedOnly().setMask(plane, tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
            else
                RSObjectsRegion.getRegion((short) tile.getRegionId(), false).forceGetRegionMap().setMask(plane, tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
            return;
        }
    ...
    public void addMask(int plane, int x, int y, int mask) {
        if (x >= 64 || y >= 64 || x < 0 || y < 0) {
            ...
            int newRegionY = (regionId & 0xff) * 64;
            if (clipedOnly)
                RSObjectsRegion.getRegion((short) tile.getRegionId(), false).forceGetRegionMapClipedOnly().addMask(plane, tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
            else
                RSObjectsRegion.getRegion((short) tile.getRegionId(), false).forceGetRegionMap().addMask(plane, tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
            return;
        }
    ...
    public void removeMask(int plane, int x, int y, int mask) {
        if (x >= 64 || y >= 64 || x < 0 || y < 0) {
            ...
            int newRegionY = (regionId & 0xff) * 64;
            if (clipedOnly)
                RSObjectsRegion.getRegion((short) tile.getRegionId(), false).forceGetRegionMapClipedOnly().removeMask(plane, tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
            else
                RSObjectsRegion.getRegion((short) tile.getRegionId(), false).forceGetRegionMap().removeMask(plane, tile.getX() - newRegionX, tile.getY() - newRegionY, mask);
            return;
        }
        masks[plane][x][y] &= (~mask);
    }
